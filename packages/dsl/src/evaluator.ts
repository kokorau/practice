...
const BUILTIN_FUNCTIONS: Record<string, BuiltinFunction> = {
  ...
  oscPulse: (args) => {
    ...
    const [t, period, arg3, arg4] = args
    // 3 args: oscPulse(t, period, duty)
    // 4 args: oscPulse(t, period, offset, duty)
    const offset = args.length === 4 ? arg3 : 0
    const duty = args.length === 4 ? arg4 : arg3
    if (period === 0) return 0
    let p = ((t + offset) % period) / period
    if (p < 0) p += 1
    return p < duty ? 1 : 0
  ...
  oscStep: (args) => {
    ...
    const offset = args.length === 4 ? arg3 : 0
    const steps = args.length === 4 ? arg4 : arg3
    if (period === 0) return 0
    let p = ((t + offset) % period) / period
    if (p < 0) p += 1
