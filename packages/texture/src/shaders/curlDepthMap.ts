...
export interface CurlDepthMapParams {
  ...
  intensity: number    // curl intensity (0.5-3), default 1
}
 ...
 *   seed: f32 (4) + contrast: f32 (4) + offset: f32 (4) + intensity: f32 (4) = 16 bytes
 *   Total: 32 bytes
 */
export const CURL_DEPTH_MAP_BUFFER_SIZE = 32

// ============================================================
// WGSL Shader
// ============================================================

export const curlDepthMapShader = /* wgsl */ `
struct Params {
  ...
  intensity: f32,    // 4 bytes @ offset 28
}                    // Total: 32 bytes
...
fn computeCurl(p: vec2f, octaves: i32) -> vec2f {
  ...
  let dx = fbmOctaves(p + vec2f(eps, 0.0), octaves) - fbmOctaves(p - vec2f(eps, 0.0), octaves);
  let dy = fbmOctaves(p + vec2f(0.0, eps), octaves) - fbmOctaves(p - vec2f(0.0, eps), octaves);
...
export function createCurlDepthMapSpec(
  params: CurlDepthMapParams,
  viewport: { width: number; height: number }
): TextureRenderSpec {
  const data = new Float32Array(CURL_DEPTH_MAP_BUFFER_SIZE / 4)
  ...
  data[7] = params.intensity

  return {
    shader: curlDepthMapShader,
    uniforms: data.buffer,
    bufferSize: CURL_DEPTH_MAP_BUFFER_SIZE,
  }
}
