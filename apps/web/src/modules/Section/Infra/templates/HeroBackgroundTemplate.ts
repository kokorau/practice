import type { SectionTemplate, HeroBackgroundContent } from '../../Domain/ValueObject/SectionTemplate'
...
<section class="section-hero-background" style="${cssVars} position: relative; min-height: 600px; display: flex; align-items: center; justify-content: center;">
  ...
  <div style="position: relative; z-index: 1; max-width: 800px; margin: 0 auto; padding: 4rem 2rem; text-align: center; color: rgb(var(--color-on-base));">
    <h1 style="margin: 0 0 1.5rem; font-size: 3.5rem; font-weight: 700; line-height: 1.1;">${c.title}</h1>
    ${c.subtitle ? `<p style="margin: 0 0 2rem; font-size: 1.25rem; opacity: 0.9; line-height: 1.6;">${c.subtitle}</p>` : ''}
