import type { SectionTemplate, FeatureCardContent } from '../../Domain/ValueObject/SectionTemplate'
...
export const FeatureCardTemplate: SectionTemplate = {
  ...
  render: (content, cssVars) => {
    ...
    const cards = c.cards
      ...
      .map(
        (card) => `
      <div style="background: rgb(var(--color-secondary)); overflow: hidden;">
        <div style="aspect-ratio: 16/9; overflow: hidden;">
          <img src="${card.imageUrl}" alt="${card.imageAlt ?? ''}" style="width: 100%; height: 100%; object-fit: cover;" />
        </div>
        <div style="padding: 1.5rem; color: rgb(var(--color-on-secondary));">
          <h3 style="margin: 0 0 0.5rem; font-size: 1.25rem; font-weight: 600;">${card.title}</h3>
          <p style="margin: 0; opacity: 0.8; line-height: 1.6;">${card.description}</p>
        </div>
      </div>`
      )
      .join('')
